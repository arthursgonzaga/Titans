 ...
 * **************************************************************************
     ...
     um sistema em Tempo Real com tempo de atualização à 50ms.
 ************************************************************************/
// PWM Left (P2.1)
// PWM Right (P2.4)
...
// Enables (P1.X):
#define left_forward BIT3  // in-4 - inputs para a ponte h
#define left_reverse BIT0  // in-3
#define right_forward BIT5 // in-2
#define right_reverse BIT4 // in-1

#define L TA1CCR1    // PWM 1 (duty cycle)
#define R TA1CCR2    // PWM 2 (duty cycle)
...
// Configurações principais encontram-se na main
 int main(void) {
    ...
    for(;;){
        ...
        coord_X = map((long)Pulse_1,950,2200,-1550,1550);
        coord_Y = map((long)Pulse_2,950,2200,-1550,1550);

        if(abs(coord_X) < 50 && abs(coord_Y) < 50){ // --> Nessa configuração, valores entre os intervalos(-20 -> 20) serão considerados como Zero.
